#!/usr/bin/env bash
#
# Build music-ffi for iOS and assemble build/core/MusicCore.xcframework plus
# the committed Swift/header bindings.
set -euo pipefail

readonly DEVICE_TARGET="aarch64-apple-ios"
readonly SIM_TARGET="aarch64-apple-ios-sim"
readonly LIB_NAME="libmusic_ffi.a"
readonly DYLIB_NAME="libmusic_ffi.dylib"
readonly MODULE="MusicCore"
...
build_target() {
    ...
    (cd "$CORE_DIR" && cargo build --locked -p music-ffi --target "$target" --profile "$CARGO_PROFILE")
...
if [[ -z "$BINDGEN_DYLIB" ]]; then
    echo "error: no music-ffi dylib from this build" >&2
    exit 1
fi
...
cp "$STAGING/$MODULE.swift" "$ROOT/LocalMusic/MusicCore.swift"
cp "$STAGING/${MODULE}FFI.h" "$ROOT/LocalMusic/MusicCoreFFI.h"
