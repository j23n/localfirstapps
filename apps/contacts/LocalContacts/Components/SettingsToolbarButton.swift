...
/// Gear button used by the top-level toolbar. Presents the Settings sheet via
/// the binding the call site already owns; overlays a small red dot when a
/// crash payload is pending so the user notices the in-Settings banner
/// without us pushing a full notification.
///
