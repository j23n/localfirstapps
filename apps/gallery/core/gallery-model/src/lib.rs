...
//!
//! [`photo`] mirrors `PhotoFile`/`PhotoFolder` field for field,
//! [`snapshot`] mirrors `JSONDiskCache<LibrarySnapshot>` key for key,
//! and [`date`] / [`file_url`] exist because Foundation's `Date` and
//! `URL` encodings are easy to approximate and expensive to get wrong.
