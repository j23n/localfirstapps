#!/usr/bin/env bash
#
# Generate the synthetic library (if needed) and run the ignored 20k e2e
# suite. Never invoked from PR CI — see .github/workflows/e2e-20k.yml
...
echo "==> gallery-scan e2e_generated_library"
cargo test --locked --release -p gallery-scan --test e2e_generated_library \
    generated_library_scan_cold_and_light \
    -- --ignored --nocapture --exact

echo "==> gallery-memories perf"
cargo test --locked --release -p gallery-memories --test perf \
    -- --ignored --nocapture
