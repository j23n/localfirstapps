...
anything distributed ships the tagging variant (see README). SFace + YuNet is
a licence-compatible candidate, not a selected replacement; keep
`PACK_VARIANT=full|tagging` until product measurements support a change.
...
`build_core.sh` on a machine downloads pyke's prebuilt static
`libonnxruntime.a` (~85 MB) into `~/Library/Caches/ort.pyke.io/`. This does
not permit runtime networking.
...
Cargo workspace behind a UniFFI boundary. Standing decisions: `docs/adr/`.
Crates:
...
- **gallery-meta** — XMP sidecar read/write. Preservation-first
  ...
  HEIC XMP is read by `media/isobmff.rs`: a bounded ISO-BMFF box walk that finds a
...
- **gallery-ml** — `TaggingEngine`: content hash → embedding cache → pinned
  ...
  `merge_clusters` / `split_cluster` / `sync_sidecars`) and the auto-tag pass
  that extends an already-named cluster to newly matched faces. Merge and split
  (Phase 5) are bookkeeping over the partition plus a sidecar re-derivation —
  nothing is re-detected or re-embedded — and both **pin** the clusters they
  touch (cache schema v4, additive `ALTER TABLE`). The pin is the whole point:
  `recluster()` throws the unlabeled partition away and rebuilds it, so without
  it a hand-made merge would be silently undone by the next full pass.
  `unname_cluster` clears it, because "back in play for the full pass" has to
  keep meaning what it says. A sibling rather than an extension: the two queues
...
- **gallery-memories** — `MemoryEngine` + `computeScheduledMemories`, ported
  ...
  the horizon cost: people → photos is grouped **once per call** rather
  ...
  The one exception is the id's date, **fixed** in the current rule: it names the
  local calendar day the memory is about (`LocalCalendar::iso_day`), where it
  used to render the instant in GMT, so the widget's pre-published id for a day
  and the id the rail generates on that day agree by construction. The horizon
  walks **civil** days for it, and its validity windows use a per-day offset
  table (`Zone::at_horizon_day`, filled by `CoreMemories.horizonOffsets`) so a
  DST transition inside the seven days cannot reopen it — the window's calendar
  and the generators' calendar are deliberately different, and collapsing them
  back into one brings the bug back.
- **gallery-ffi** — the sole crate the app sees (proc-macro UniFFI, namespace
  ...
  name/unname/ignore/rename/merge/split/dismissProposal/recluster/resetQueue —
  refuses with `AlreadyRunning` while a run is in flight** rather than blocking
  the caller or racing the run's own auto-tag pass over the cluster table;
  reads (`clusters`, `clusterFaces`, `mergeProposals`) stay open so a review
  screen does not go blank for the length of a scan. `ClusterSummary` carries
  ≤4 exemplar `FaceRef`s — photo path plus a **normalized MWG centre/extent
  rectangle**, chosen best quality first and one per photo where possible — so
  Swift crops from its own thumbnail pipeline and no pixels cross the boundary.
  Phase 5 adds `mergeClusters(into:from:)`, `splitCluster(clusterId:faceKeys:)`
  and `mergeProposals()`/`dismissMergeProposal(a:b:)`. `FaceRef.faceKey` is
  what a split acts on — `<content-hash-hex>:<idx>`, opaque to Swift, because a
  rectangle is not an identity: two detections in one photo can overlap, so a
  selection made by geometry can move the wrong face. Malformed keys are
  `InvalidFaceKey`; a key whose row is merely *gone* is counted in
  `SplitResult.ignoredKeys` and skipped, since a review screen open across a
  run is ordinary.
...
and git-ignored. `scripts/generate_bindings.sh` refreshes the Swift and
header without Xcode; CI fails on drift. The Swift/Rust parity contract is the shared
...
- **PeopleStore** — hidden/featured/me person state, visible people lists, per-person cover photo + face-region matching, own UserDefaults persistence. Cross-domain side effects (memory regen, widget export) are Store-injected closures. **Every persisted key is a person's tag path**, so `renamePerson(from:to:)` exists and `GalleryStore.migratePersonState` calls it (plus the fifth path-keyed value, `personContactLinks`) on every rename — miss it and the user silently loses their "me" person, pins, hidden set and cover photos, with nothing to notice until they go looking.
...
- **LibrarySnapshot** — the persisted scan result (`rootFolder` + `allPhotos` + an optional `sidecarManifest`) and its schema `version` history; `MemoriesCacheSchema` (independent version) lives alongside. **`sidecarManifest` was added to v20 without a version bump** (docs/adr/0002): a v20 file written before it existed decodes with `nil`, pays one legacy re-probe, then persists it. Bumping instead would have cost every installed library a full rescan to save one pass. `GalleryStore.loadCache()` seeds `lastSidecarManifest` from it *before* `restoreFolder` starts the launch scan — that ordering is the whole fix. The same file is read and written by the Rust core. `DownloadStatus` and `ContentVersion.contentIdentifier` remain tolerant-decode compatibility debt for old v20 snapshots, not a provider lifecycle the product may extend.
- **LogStore** / **LogRedactor** — opt-in, per-device in-memory diagnostic capture feeding `LogsView`; it is separate from the synced `.gallery/log/<dev>/` domain event log and has no MetricKit or automatic upload. There is no disk persistence while local diagnostic capture is disabled. `Log.r.*` tokenizes identifying strings; **use `Log.r.error(_:)` instead of `error.localizedDescription`** in log interpolation (Cocoa errors embed file names).
...
- **SidecarCacheStore** / **SidecarSyncService** / **SidecarRefreshService** — parsed `.xmp` cache + BG refresh. Locally read bytes are parsed by the core (`parseXmpBytes`), the same parser the scan and enrichment paths use; these services do not materialise provider items.
...
- **Viewer/** — `PhotoViewerView` (chrome + filmstrip), `PagingPhotoView` (UIPageViewController wrapper), `PhotoPageView` (one page incl. video/live plus legacy non-materialising compatibility states), `ZoomableImageView`, `SwipeToDismissGesture`.
- **Collections/** — `PersonCard`, `PersonContextMenu` (shared Me/Feature/Link/Hide menu), `MemoryCardView`, `MemoryGridView`, `TagGridView`, `PeopleListView`, and the Phase 2 face review: `PeopleReviewRow` (the People-screen doorway, shown only when `faces.reviewableClusters` is non-empty) → `PeopleReviewView` (cluster grid) → `ClusterReviewView` (every face, a name field with contact/library suggestions, "Not a Person", "Merge with…" via `ClusterPickerView`, and a Select mode over the face grid that splits the chosen faces out). `PeopleReviewView` also carries a "Suggested Merges" section (`MergeSuggestionRow`) over the core's proposals. Crops are `PersonThumbnailView` with the region the core returned — no new crop path, because `FaceRef` is already MWG-shaped. **`MergeDirection` is the one place that decides which of two groups survives** (named beats unnamed, then bigger, ties on id) and writes the sentence that names the dropped name; both merge entry points go through it, or the same pair would merge differently depending on where the user tapped. `RenamePersonSheet` (reached from `PersonContextMenu`, hidden when no face models are installed) renames a person everywhere, confirming the combine when the new name already exists.
...
- **Kinds**: `.light` (pull-to-refresh; reuses cached `PhotoFile`s for unchanged files — one stat per file, no EXIF), `.full` (Settings "Reload Library"; rebuilds everything), `.auto` (foreground/cold-launch; light, promoted to full when >48h since the last full pass — `fullScanInterval`).
...
- **Phase 1 / Phase 2:** no provider lifecycle surface. The scanner accepts
  entries with local bytes (`ScannerSession()`); placeholders do not enter
  the projection and the app never requests materialisation. Provider-named
  snapshot/UI branches that remain are compatibility debt pinned by
  `conformance/semantic`, not behavior to preserve or extend.
...
| Sidecars (`SidecarCacheStore.version`) | own version bump, per-entry content-version diff, GC of orphans after scan; the "Re-download" label is legacy provider-language debt |
| Thumbnails (disk) | source mtime newer than cache entry; "Clear thumbnail cache"; QuickLook placeholder branches are legacy debt, not a materialisation contract |
| `.nothumb` sentinels | legacy QuickLook-path compatibility, expiring after 24h; remove with the provider-placeholder debt |
...
- Merge proposals are only recomputed by a run, and a run recomputes them wholesale — so "Not the same" survives until the next scan and no longer. A dismissal that outlived a re-cluster would need a table of its own.
