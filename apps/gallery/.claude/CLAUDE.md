...
- **LogPersistence** / **LogStore** / **LogRedactor** — in-app ring-buffer log capture feeding `LogsView`. `LogPersistence` is a no-op until a later local-only log setting exists. `Log.r.*` tokenizes identifying strings; **use `Log.r.error(_:)` instead of `error.localizedDescription`** in log interpolation (Cocoa errors embed file names).
...
- **SidecarCacheStore** / **SidecarSyncService** / **SidecarRefreshService** — parsed `.xmp` cache + BG refresh. Fetched bytes are parsed by the core (`parseXmpBytes`), the same parser the scan and enrichment paths use.
...
- **Phase 1:** no File Provider surface. `LocalOnlyProbe` answers the FFI; placeholders do not enter the projection. Do not restore per-file provider keys.
