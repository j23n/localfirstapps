...
- **LogStore** / **LogRedactor** — in-memory ring-buffer log capture feeding `LogsView`; there is no disk persistence while local diagnostic capture is disabled. `Log.r.*` tokenizes identifying strings; **use `Log.r.error(_:)` instead of `error.localizedDescription`** in log interpolation (Cocoa errors embed file names).
