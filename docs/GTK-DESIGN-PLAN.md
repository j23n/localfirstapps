...
| collections | Root. **Memories** carousel of hero cards (scrim, Newsreader Italic title, dim caption) + dots. **People**: card tiles, name + count on scrim. **Events / Albums**: rails | Memories as a rail of 2–3 hero cards; People 5–6 columns |
...
| viewer | Full bleed, `osd` bars, auto-hide, swipe/zoom. Overflow: Info, Open With, Show in Folder | `AdwOverlaySplitView` with photo-info sidebar |
| photo-info | `AdwBottomSheet` (compact): Date, Camera, Location (place name, no map), Tags, People | sidebar |
