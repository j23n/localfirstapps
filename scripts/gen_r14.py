...
SCREEN_SWIFT = {
    "gallery": REPO / "apps/gallery/LocalGallery/Generated/Screens.swift",
...
def display_font(table: dict) -> dict | None:
    """Optional `[fonts] display = { family, style, file }` (ADR 0004 R11)."""
    fonts = table.get("fonts") or {}
    display = fonts.get("display")
    if not display:
        return None
    if not isinstance(display, dict):
        raise SystemExit("error: [fonts] display must be a table")
    missing = [key for key in ("family", "style", "file") if not display.get(key)]
    if missing:
        raise SystemExit(
            "error: [fonts] display missing " + ", ".join(missing)
        )
    return display
...
def generate_tokens_rs(tables: dict[str, dict]) -> str:
    ...
    for app, table in tables.items():
        ...
        if font := display_font(table):
            lines.append(
                f'    pub const DISPLAY_FAMILY: &str = "{font["family"]}";'
            )
            lines.append(
                f'    pub const DISPLAY_STYLE: &str = "{font["style"]}";'
            )
            lines.append(f'    pub const DISPLAY_FILE: &str = "{font["file"]}";')
        lines.append("}")
        lines.append("")
    return "\n".join(lines)
...
def generate_tokens_swift(app: str, table: dict) -> str:
    ...
    if font := display_font(table):
        lines.append(f'    static let displayFamily = "{font["family"]}"')
        lines.append(f'    static let displayStyle = "{font["style"]}"')
        lines.append(f'    static let displayFile = "{font["file"]}"')
    lines.append("}")
    lines.append("")
    return "\n".join(lines)
...
def generate_tokens_css(app: str, table: dict) -> str | None:
    ...
    if not colors and not metrics:
        return None
    # Fonts are constants (Rust/Swift) only. Never emit font-family here.
    ...
    if "font-family" in css:
        raise SystemExit(
            f"error: {app} generated CSS must not contain font-family"
        )
    return css
