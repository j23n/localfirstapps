...
Light-only until Phase 3.5: a `dark` key is an error.
"""
...
def refuse_dark(app: str, role: str, spec: dict) -> None:
    if spec.get("dark"):
        raise SystemExit(
            f"error: dark companions are Phase 3.5; remove dark from "
            f"design/tokens/{app}.toml [{role}]"
        )
...
def generate_tokens_rs(tables: dict[str, dict]) -> str:
    lines = [
        "//! Generated by `scripts/gen_r14.py`. Do not edit.",
        "//! ADR 0004 R11 token tables. Light hex only (Phase 3.5 adds dark).",
        "",
    ]
